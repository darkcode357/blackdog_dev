...
# If in automatic mode, determine if a user already exists, if not use blackdog
if [ "${USERNAME}" = "auto" ] || [ "${USERNAME}" = "automatic" ]; then
    USERNAME=""
    POSSIBLE_USERS=("blackdog" "node" "codespace" "$(awk -v val=1000 -F ":" '$3==val{print $1}' /etc/passwd)")
...
if [ -t 1 ] && [[ "${TERM_PROGRAM}" = "blackdog" || "${TERM_PROGRAM}" = "codespaces" ]] && [ ! -f "$HOME/.config/vscode-dev-containers/first-run-notice-already-displayed" ]; then
...
if [ -f "${SCRIPT_DIR}/meta.env" ]; then
    mkdir -p /usr/local/etc/blackdog_container/
    cp -f "${SCRIPT_DIR}/meta.env" /usr/local/etc/blackdog_container/meta.env
